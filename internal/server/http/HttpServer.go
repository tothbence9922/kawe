...
func getQueriedPods(nameParam string, statusParam string) ([](processorInterfaces.IProcessedData), error) {

	ag := aggregator.GetInstance()
	ag.Lock()
	defer ag.Unlock()

	results := make([](processorInterfaces.IProcessedData), 0)
...
func handleQueryPods(req *http.Request) ([]byte, error) {

	statusQuery := req.URL.Query().Get("status")
	nameQuery := req.URL.Query().Get("name")

	results, err := getQueriedPods(nameQuery, statusQuery)
...
func handleGetPods(w http.ResponseWriter, req *http.Request) {
	response, _ := handleQueryPods(req)
...
func (hs HttpServer) Serve(wg *sync.WaitGroup) {
	...
	go func() {
		...
		http.HandleFunc("/api/v1/pods", handleGetPods)
